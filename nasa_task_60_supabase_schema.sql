-- NASA_task_60 専用 Supabase スキーマ
-- Supabase Dashboard > SQL Editor で実行してください。

create extension if not exists pgcrypto;

create table if not exists public.nasa_60_submissions (
...
create table if not exists public.nasa_60_responses (
  id bigint generated always as identity primary key,
  submission_id uuid not null references public.nasa_60_submissions(id) on delete cascade,
...
create index if not exists idx_nasa_60_participant
  on public.nasa_60_submissions (participant_id);
create index if not exists idx_nasa_60_duration
  on public.nasa_60_submissions (viewing_duration);
create index if not exists idx_nasa_60_responses_submission
  on public.nasa_60_responses (submission_id);

comment on column public.nasa_60_submissions.raw_tlx_mean is
  '精神的要求・身体的要求・時間的要求・パフォーマンス・努力・フラストレーションの単純平均（0～100）';
comment on column public.nasa_60_submissions.overall_workload is
  '追加項目「全体的負荷」の回答値（Raw TLX平均には含めない）';
comment on column public.nasa_60_responses.cumulative_duration_sec is
  '項目表示から次へ/戻るまでの時間を訪問ごとに合算（修正時の再訪を含む）';
comment on column public.nasa_60_responses.latency_to_first_input_sec is
  '各訪問で項目表示から最初にスライダーを操作するまでの時間の合計';

alter table public.nasa_60_submissions enable row level security;
alter table public.nasa_60_responses enable row level security;

create or replace function public.submit_nasa_60_survey(payload jsonb)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.nasa_60_submissions (
  ...
  insert into public.nasa_60_responses (
...
revoke all on function public.submit_nasa_60_survey(jsonb) from public;
grant execute on function public.submit_nasa_60_survey(jsonb) to anon, authenticated;
revoke all on table public.nasa_60_submissions from anon, authenticated;
revoke all on table public.nasa_60_responses from anon, authenticated;
