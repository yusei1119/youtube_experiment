"""SupabaseのNASA_task_90回答を分析用CSVへ出力する。
...
使用例:
  python -m scripts.export.export_nasa_90_responses
  python -m scripts.export.export_nasa_90_responses --format long
"""
...
PAGE_SIZE = 1000
DEFAULT_OUTPUT = "data/exports/nasa_task_90_results.csv"
DIMENSION_ORDER = (
...
def flatten_wide(submissions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    rows.sort(key=lambda row: (
        ...
        int(row.get("condition_order") or 0),
    ))
    return rows
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NASA_task_90の回答をCSV出力")
    parser.add_argument(
        "--output", "-o", default=DEFAULT_OUTPUT,
        help="基準出力名（実際のファイル名には実行日時を付加）",
    )
...
def main() -> None:
    ...
    params = {
        "select": (
            "id,survey_id,participant_id,video_condition,condition_order,total_conditions,"
            "page_session_id,started_at,completed_at,total_duration_sec,raw_tlx_sum,"
            "raw_tlx_mean,overall_workload,created_at,nasa_90_responses(*)"
        ),
        "order": "created_at.asc",
        "nasa_90_responses.order": "display_order.asc",
    }
    endpoint = f"{base_url.rstrip('/')}/rest/v1/nasa_90_submissions"
    ...
    output = versioned_file(args.output, args.run_id)
