...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-90", type=Path,
        default=Path("data/local_exports/writing_responses.csv"),
    )
    parser.add_argument(
        "--input-60", type=Path,
        default=Path("data/local_exports/writing_60_responses.csv"),
    )
    parser.add_argument(
        "--nasa-90-input",
        type=Path,
        default=Path("data/local_exports/nasa_task_90_results.csv"),
        help="90版の条件ラベルを時系列照合するNASA CSV",
    )
    parser.add_argument(
        "--output-dir", type=Path,
        default=Path("analysis_runs/individual/writing"),
