...
実行例:
  python -m scripts.analysis.summarize_youtube_viewing_60 \
    --youtube-input <集計CSV> --duration-input <NASA 60 CSV>
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--duration-input",
        type=Path,
        default=Path("data/local_exports/nasa_task_60_results.csv"),
        help="participant_idとviewing_durationを持つCSV",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("analysis_runs/individual/youtube/b_participants"),
        help="出力基準ディレクトリ（内部に run_実行日時 を自動作成）",
    )
