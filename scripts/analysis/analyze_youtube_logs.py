...
参加者サマリーに含める指標（ご要望）:
  ...
  - total_view_sec            総視聴時間（全動画の累積再生秒数の合計）
...
# ---- 判定のしきい値（必要なら変更） --------------------------------------
COMPLETION_RATE = 0.9   # 視聴完了とみなす視聴到達率（max_reached_sec / duration_sec）
EARLY_SKIP_SEC = 2.0    # 早期スキップとみなす視聴秒数の上限（これ以下なら早期スキップ）
SEEK_JUMP_SEC = 3.0     # これを超える current_time_sec の急増はシークとして扱い、時刻差で抑える
# -------------------------------------------------------------------------

NEXT_EVENTS = {"next", "skip", "swipe_next"}
PREVIOUS_EVENTS = {"previous", "swipe_previous"}
FINISH_EVENTS = {"next_finish", "swipe_next_finish"}
...
def filter_screen_events(df):
    """画面上で active だった video_index のイベントだけを残す。"""
    rows = []

    for _, session in df.sort_values(GROUP_KEYS + ["server_time"]).groupby(GROUP_KEYS, dropna=False):
        active_index = None
        ...
        for _, row in session.iterrows():
            ...
            if active_index is None and pd.notna(video_index):
                active_index = video_index

            is_active_event = pd.notna(video_index) and video_index == active_index
            if is_active_event:
                rows.append(row)

            if is_active_event and event_type in NEXT_EVENTS:
                active_index = video_index + 1
            elif is_active_event and event_type in PREVIOUS_EVENTS:
                active_index = max(video_index - 1, 0)
            elif is_active_event and event_type in FINISH_EVENTS:
                active_index = None
...
def _estimate_playback_sec(group):
    """current_time_sec の増分を足し、巻き戻し・ループ後の再視聴も累積する。"""
    g = group.sort_values("server_time")
    previous_current = np.nan
    previous_time = pd.NaT
    total_sec = 0.0

    for row in g.itertuples(index=False):
        current = row.current_time_sec
        current_time = row.server_time

        if pd.isna(current) or current < 0:
            previous_time = current_time
            continue

        if pd.isna(previous_current):
            increment = current
        elif current >= previous_current:
            increment = current - previous_current
        else:
            increment = current

        if increment > SEEK_JUMP_SEC and pd.notna(previous_time) and pd.notna(current_time):
            elapsed_sec = (current_time - previous_time).total_seconds()
            if elapsed_sec >= 0:
                increment = min(increment, elapsed_sec)
...
def build_video_table(df):
    ...
    ended = (
        screen_df[screen_df["event_type"] == "ended"]
    ...
    playback = (
        screen_df.groupby(GROUP_KEYS + ["video_index", "video_id", "video_title"], dropna=False)
        .apply(_estimate_playback_sec, include_groups=False)
        .rename("watched_sec")
        .reset_index()
    )
    ...
    video = (
        screen_df.groupby(GROUP_KEYS + ["video_index", "video_id", "video_title"], dropna=False)
        .agg(
            max_reached_sec=("max_time_sec", "max"),  # その動画で最も先まで見た位置
    ...
    video = video.merge(playback, on=GROUP_KEYS + ["video_index", "video_id", "video_title"], how="left")
    ...
    video["max_reached_sec"] = video["max_reached_sec"].fillna(0.0)

    # 視聴到達率（0〜1にクリップ）
    video["view_rate"] = np.where(
        video["duration_sec"] > 0,
        (video["max_reached_sec"] / video["duration_sec"]).clip(upper=1.0),
        np.nan,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="YouTube視聴ログを参加者単位に集計")
    parser.add_argument("input", nargs="?", type=Path, default=Path(LOG_PATH))
    parser.add_argument("--sessions", type=Path, default=Path(SESSION_PATH))
    parser.add_argument(
        "--category-cache", type=Path, default=Path(VIDEO_CATEGORY_CACHE_CSV)
    )
    parser.add_argument(
...
def main():
    global SESSION_PATH, VIDEO_CATEGORY_CACHE_CSV, CATEGORY_LABEL_CSV
    args = parse_args()
    SESSION_PATH = str(args.sessions)
